...
import RealTimeAnalytics from '../analytics/services/realTimeAnalytics';
import AIEngine, { AIInsight, AIAction } from '../ai/services/aiEngine';
...
const DonorDiscovery: React.FC = () => {
  ...
  const { setSelectedOpportunity: setContextOpportunity, setIsFromDiscovery } = useOpportunity();
  ...
  useEffect(() => {
    console.log('Modal state changed:', showApplyModal);
  }, [showApplyModal]);
  
  // Debug logging for proposal editor state
  useEffect(() => {
    console.log('Proposal editor state changed:', showProposalEditor);
  }, [showProposalEditor]);
  ...
  // User profile and proposal editor state
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
            ...
            >
              ...
              <motion.button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  console.log('🚀 INSTANT STREAM - Apply Now clicked, selectedOpportunity:', selectedOpportunity);
                  
                  if (selectedOpportunity) {
                    // Start content generation immediately in background
                    console.log('⚡ Pre-generating content while user sees modal...');
                    
                    // Start instant preload generation
                    fetch('/api/ai/preload-content', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({
                        opportunity: selectedOpportunity,
                        sections: ['Executive Summary', 'Organization Background', 'Problem Statement', 'Project Description']
                      })
                    }).catch(err => console.log('⚡ Preload started in background'));
                    ...
                    const opportunityData = {
                      ...
                      amountMin: Math.floor(selectedOpportunity.averageAmount * 0.8),
                      amountMax: Math.floor(selectedOpportunity.averageAmount * 1.2),
                      currency: 'USD',
                      deadline: selectedOpportunity.deadline,
                      sourceUrl: selectedOpportunity.sourceUrl,
                      sourceName: selectedOpportunity.sourceName,
                      isVerified: selectedOpportunity.verified,
                      requirements: selectedOpportunity.requirements,
                      funderPriorities: selectedOpportunity.aiSuggestions,
                      fundingType: selectedOpportunity.fundingType,
                      tags: selectedOpportunity.tags
                    };
                    
                    // Set opportunity context for seamless flow
                    setContextOpportunity(opportunityData);
                    setIsFromDiscovery(true);
                    
                    console.log('Setting showApplyModal to true');
                    setShowApplyModal(true);
                  } else {
                    console.log('No selectedOpportunity');
                  }
                }}
                transition={{ 
                  type: "spring", 
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <motion.button
                  ...
                  onClick={async (e) => {
                    e.stopPropagation();
                    console.log('🚀 AI FULL CONTROL - FETCHING USER DATABASE PROFILE');
                    console.log('📊 Using real user data for intelligent automation');
                    console.log('🤖 No questions asked - complete automation with database profile');
                    ...
                    try {
                      ...
                      if (profileData.success) {
                        console.log('✅ User profile loaded from database:', profileData.profile.organization_name);
                        
                        // Set enhanced opportunity context with real user data
                        const enhancedContext = {
                          ...selectedOpportunity,
                          auto_start_generation: true,
                          skip_all_questions: true,
                          stream_immediately: true,
                          use_database_profile: true,
                          user_profile: profileData.profile
                        };
                        
                        setContextOpportunity(enhancedContext);
                        setIsFromDiscovery(true);
                        
                        // Close modal first, then open Word processor
                        setShowApplyModal(false);
                        console.log('🔄 Opening Word processor in 100ms...');
                        setTimeout(() => {
                          console.log('📝 Setting showProposalEditor to true');
                          setShowProposalEditor(true);
                        }, 100);
                        
                        trackInteraction.mutate({ type: 'database_ai_automation', credits: 8 });
                      } else {
                        console.error('❌ Failed to load user profile');
                        // Still use Word processor even without profile
                        setShowApplyModal(false);
                        console.log('🔄 Opening Word processor (no profile) in 100ms...');
                        setTimeout(() => {
                          console.log('📝 Setting showProposalEditor to true (no profile)');
                          setShowProposalEditor(true);
                        }, 100);
                      }
                    } catch (error) {
                  ...
                  className="w-full flex items-center gap-4 p-6 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 border-2 border-purple-200 dark:border-purple-800 rounded-2xl hover:border-purple-300 dark:hover:border-purple-700 transition-all group relative overflow-hidden"
                >
                  {/* Animated Background */}
                  <motion.div
                    className="absolute inset-0 bg-gradient-to-r from-purple-400 to-pink-400 opacity-10"
                    animate={{
                      x: [-100, 100, -100],
                    }}
                    transition={{
                  ...
                  <div className="w-12 h-12 bg-purple-500 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform relative z-10">
                    <motion.div
                      animate={{ rotate: 360 }}
                  ...
                  <div className="flex-1 text-left relative z-10">
                    <h4 className="font-semibold text-gray-900 dark:text-white mb-1">
                      🚀 AI Takes Complete Control
                    </h4>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      Full automation with charts, tables & scenarios
                    </p>
                    <div className="text-xs text-purple-600 dark:text-purple-400 mt-1 flex items-center gap-2">
                      <span>Uses 8 credits</span>
                      <span className="bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full">AUTOMATED</span>
                    </div>
                  </div>
                  <motion.div
                    animate={{ x: [0, 5, 0] }}
                ...
                <motion.button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    // Use Word processor for document formatting
                    setShowApplyModal(false);
                    setTimeout(() => {
                      setShowProposalEditor(true);
                    }, 100);
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      Format existing documents to requirements
                    </p>
                ...
                <motion.button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    // Connect with expert help while using Word processor
      ...
      {/* User Profile Selector for Testing */}
      <UserProfileSelector
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="w-full h-full max-w-7xl mx-auto">
            <IntelligentProposalEditor
              sections={[
                { id: '1', title: 'Executive Summary', content: '', required: true },
                { id: '2', title: 'Project Description', content: '', required: true },
                { id: '3', title: 'Budget Overview', content: '', required: true },
                { id: '4', title: 'Implementation Timeline', content: '', required: true },
                { id: '5', title: 'Expected Impact', content: '', required: true },
                { id: '6', title: 'Sustainability Plan', content: '', required: true }
              ]}
              onSectionsUpdate={() => {}}
              opportunityContext={selectedOpportunity}
              onClose={() => {
                console.log('🚪 Closing Word processor');
