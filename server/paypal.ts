...
export async function getClientToken() {
  if (!isPayPalConfigured || !oAuthAuthorizationController) {
    throw new Error("PayPal not configured. Please add PAYPAL_CLIENT_ID and PAYPAL_CLIENT_SECRET to your environment variables.");
  }
  ...
  try {
    if (!isPayPalConfigured || !ordersController) {
      return res.status(400).json({
        error: "PayPal not configured. Please contact administrator to add PayPal credentials.",
      });
    }
...
export async function loadPaypalDefault(req: Request, res: Response) {
  try {
    ...
    res.status(500).json({
      error: "Failed to initialize PayPal. Please try again later.",
    });
  }
}
// <END_EXACT_CODE>